...
| C1 concurrency | `IN_PROGRESS` | 9 / 15 | 12 / 30 | 1 configuration failure, 2 observability rejections | D/A-first PASS; fallback then re-entry final External |
...
Record and push `C1-D-A_FIRST`, confirm clean synchronization, then run
`C1-D-NEAR_SIMULTANEOUS` attempt 1 with seed `320411`.
