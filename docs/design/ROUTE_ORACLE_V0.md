...
`exclusivity` delegates candidate completeness, stable writer IDs, publisher sequence, rate, transition-window coverage, competing writers, and holes to `actuator_writer_collector.py`. `COMPETING_WRITERS` is a violation; any coverage state other than `EXCLUSIVE` is unknown.
...
PX4 boot/uORB/ULog microseconds from one boot are directly comparable, including multiple ULog segments caused by disarm/rearm. ROS-node timestamps are never subtracted from PX4 time unless the result supplies `clock_bridge_id`, offset, uncertainty, and a two-ended validity interval. Producer-to-PX4 latency is unknown when that bridge is absent or invalid.
