...
def run_one(
    ...
    try:
        ...
        with console_log.open("w", encoding="utf-8") as console_handle, open(cmd_tmp.name, "r", encoding="utf-8") as stdin:
            ...
            with task_log.open("w", encoding="utf-8") as task_handle:
                task = subprocess.Popen(
                    [
                        sys.executable,
                        str(repo / "scripts/probes/m1_offboard_task.py"),
                        "--theta",
                        str(theta_path),
                        "--controller",
                        controller,
        ...
        metrics_cmd = [
            sys.executable,
            str(repo / "scripts/analysis/m1_metrics.py"),
            "--ulog",
            str(copied_ulog),
            "--theta",
            str(theta_path),
...
def main() -> int:
    ...
    parser.add_argument("--docs-dir", type=Path, default=repo_root() / "runs/family_b")
    ...
    if not args.skip_build:
        ...
        run_checked([str(repo / "scripts/setup/build_px4_raptor_sih.sh")], cwd=repo, log=docs / f"m1_{tag}_build.log", env=build_env)
    else:
        run_checked([str(repo / "scripts/setup/install_raptor_sih_board.sh")], cwd=repo, log=docs / f"m1_{tag}_build.log", env=env)
        run_checked([str(repo / "scripts/setup/install_m1_sih_x500.sh")], cwd=repo, log=docs / f"m1_{tag}_build.log", env=env)
        run_checked([str(repo / "scripts/tracing/install_dds_groundtruth.sh")], cwd=repo, log=docs / f"m1_{tag}_build.log", env=env)
        run_checked([str(repo / "scripts/setup/install_m2b_state_shim.sh")], cwd=repo, log=docs / f"m1_{tag}_build.log", env=env)
    ...
    run_checked(
        [
            sys.executable,
            str(repo / "scripts/analysis/m1_compare.py"),
            "--theta",
            str(theta_path),
            "--classical",
