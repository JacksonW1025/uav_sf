...
RUN_ID="${R1_RUN_ID:?R1_RUN_ID is required}"
SCENARIO="${R1_SCENARIO:?R1_SCENARIO is required}"
SIMULATION_SEED="${R1_SIMULATION_SEED:?R1_SIMULATION_SEED is required}"
[[ "${SCENARIO}" =~ ^(A|B|C)$ ]] || { echo "invalid R1 scenario" >&2; exit 2; }
[[ "${SIMULATION_SEED}" =~ ^[0-9]+$ ]] || { echo "invalid R1 simulation seed" >&2; exit 2; }
...
source "${R1_WORKSPACE_SETUP:-${REPO_ROOT}/runs/r1_harness_build/install/setup.bash}"
set -u

PX4_DIR="${PX4_R1_DIR:-${REPO_ROOT}/external/PX4-Autopilot-oracle-validation-control}"
PX4_BUILD="${PX4_DIR}/build/px4_sitl_default"
AGENT_BUILD="${R1_AGENT_BUILD:-${REPO_ROOT}/runs/freshness_agent_build}"
...
MODE_BIN="${R1_MODE_BIN:-${REPO_ROOT}/runs/r1_harness_build/install/r1_session_adapter/lib/r1_session_adapter/r1_session_probe}"
LOGGER_TOPICS_FILE="${R1_LOGGER_TOPICS_FILE:-${REPO_ROOT}/config/freshness_logger_topics.txt}"
RAW_DIR="${R1_RAW_ROOT:-${REPO_ROOT}/runs/motivation/r1_session/${RUN_ID}/raw}"
PROCESSED_DIR="${R1_PROCESSED_ROOT:-${RAW_DIR}/processed}"
CONTROL_DIR="${RAW_DIR}/session_control"

[[ -x "${PX4_BUILD}/bin/px4" ]] || { echo "R1 PX4 build unavailable" >&2; exit 4; }
[[ -x "${AGENT_BIN}" ]] || { echo "R1 Micro-XRCE-DDS Agent unavailable" >&2; exit 4; }
[[ -x "${MODE_BIN}" ]] || { echo "R1 mode binary unavailable" >&2; exit 4; }
[[ -f "${LOGGER_TOPICS_FILE}" ]] || { echo "R1 logger profile unavailable" >&2; exit 4; }
[[ ! -e "${RAW_DIR}" ]] || { echo "refusing to overwrite R1 raw attempt" >&2; exit 3; }
...
MONITOR_PID=""
GZ_PID=""
GZ_PARTITION_NAME="uav_sf_r1_${RUN_ID//[^a-zA-Z0-9_]/_}"
...
python3 "${REPO_ROOT}/scripts/probes/r1_session_monitor.py" \
  --run-id "${RUN_ID}" --scenario "${SCENARIO}" --mode-bin "${MODE_BIN}" \
  --output "${RAW_DIR}/monitor_result.json" --events "${RAW_DIR}/monitor_events.jsonl" \
  --control-dir "${CONTROL_DIR}" --old-log "${RAW_DIR}/old_session.log" \
  --new-log "${RAW_DIR}/new_session.log" --timeout 150 \
...
set -e
MONITOR_PID=""
((MONITOR_STATUS == 0)) || { echo "R1 monitor failed for ${RUN_ID}" >&2; exit "${MONITOR_STATUS}"; }
...
python3 "${REPO_ROOT}/scripts/tracing/route_trace_collector.py" \
  ...
  --producer-events "${RAW_DIR}/monitor_events.jsonl" \
  --lifecycle-log "${RAW_DIR}/old_session.log" --lifecycle-log "${RAW_DIR}/new_session.log"
...
python3 "${REPO_ROOT}/scripts/oracles/session_rollover_oracle.py" \
  ...
  --old-lifecycle-log "${RAW_DIR}/old_session.log" --new-lifecycle-log "${RAW_DIR}/new_session.log" \
  --output "${PROCESSED_DIR}/session_rollover_oracle.json"
...
echo "R1_RUN=PASS run_id=${RUN_ID} scenario=${SCENARIO}"
