...
RUN_ID="${FRESHNESS_RUN_ID:?FRESHNESS_RUN_ID is required}"
SETPOINT_TYPE="${FRESHNESS_SETPOINT_TYPE:?FRESHNESS_SETPOINT_TYPE is required}"
FAULT_TYPE="${FRESHNESS_FAULT_TYPE:?FRESHNESS_FAULT_TYPE is required}"
# Allow the open-loop attitude/rate contexts to settle before the separately
# evaluated two-second pre-fault baseline window.
STABLE_SECONDS="${FRESHNESS_STABLE_SECONDS:-5.0}"
TARGET_SECONDS="${FRESHNESS_TARGET_SECONDS:-3.0}"
RECOVERY_SECONDS="${FRESHNESS_RECOVERY_SECONDS:-2.0}"
FAULT_DELAY_SECONDS="${FRESHNESS_FAULT_DELAY_SECONDS:-0.0}"
SIMULATION_SEED="${FRESHNESS_SIMULATION_SEED:-1}"
PROFILE="${FRESHNESS_PROFILE:-}"

[[ "${SETPOINT_TYPE}" =~ ^(TRAJECTORY|ATTITUDE|RATE)$ ]] || { echo "invalid setpoint type" >&2; exit 2; }
[[ "${FAULT_TYPE}" =~ ^(TOTAL_PROCESS_STOP|SETPOINT_ONLY_STALL)$ ]] || { echo "invalid fault type" >&2; exit 2; }
...
RAW_DIR="${FRESHNESS_RAW_ROOT:-${REPO_ROOT}/runs/motivation/freshness/${RUN_ID}/raw}"
PROCESSED_DIR="${FRESHNESS_PROCESSED_ROOT:-${RAW_DIR}/processed}"
...
[[ ! -e "${PROCESSED_DIR}" ]] || { echo "refusing to overwrite existing processed attempt: ${PROCESSED_DIR}" >&2; exit 3; }
...
MONITOR_PID=""
GZ_PID=""
GZ_PARTITION_NAME="uav_sf_freshness_${RUN_ID//[^a-zA-Z0-9_]/_}"
...
python3 "${REPO_ROOT}/scripts/probes/freshness_flight_monitor.py" \
  --run-id "${RUN_ID}" --setpoint-type "${SETPOINT_TYPE}" --fault-type "${FAULT_TYPE}" \
...
UAV_SF_SETPOINT_TYPE="${SETPOINT_TYPE}" \
UAV_SF_TRAJECTORY_VX_M_S="${FRESHNESS_TRAJECTORY_VX_M_S:-0.5}" \
UAV_SF_ATTITUDE_ROLL_RAD="${FRESHNESS_ATTITUDE_ROLL_RAD:-0.12}" \
UAV_SF_RATE_ROLL_RAD_S="${FRESHNESS_RATE_ROLL_RAD_S:-0.10}" \
UAV_SF_THRUST_BODY_Z="${FRESHNESS_THRUST_BODY_Z:--0.52}" \
UAV_SF_CHANNEL_CONTROL_DIR="${CONTROL_DIR}" \
  "${PRODUCER_BIN}" >"${RAW_DIR}/external_mode.log" 2>&1 &
PRODUCER_PID=$!

python3 "${REPO_ROOT}/scripts/probes/inject_freshness_fault.py" \
  --fault-type "${FAULT_TYPE}" --pid "${PRODUCER_PID}" --ready "${READY_MARKER}" \
  --control-dir "${CONTROL_DIR}" --output "${FAULT_RECORD}" --event-log "${FAULT_EVENTS}" \
...
stop_process "${PRODUCER_PID}" INT
PRODUCER_PID=""
((MONITOR_STATUS == 0)) || { echo "freshness monitor failed for ${RUN_ID}" >&2; exit "${MONITOR_STATUS}"; }
...
analysis_args=(
  ...
  --output "${PROCESSED_DIR}/freshness_observation.json"
)
if [[ -n "${PROFILE}" ]]; then analysis_args+=(--profile "${PROFILE}"); fi
python3 "${REPO_ROOT}/scripts/analysis/summarize_freshness_run.py" "${analysis_args[@]}"
...
echo "FRESHNESS_RUN=PASS run_id=${RUN_ID} setpoint=${SETPOINT_TYPE} fault=${FAULT_TYPE}"
